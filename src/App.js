...
// questions.js
// Mock data for Google Associate Cloud Engineer exam questions.
...
const questionsData = {
    ...
    test3: [
        ...
        },
        {
            ...
            correctAnswer: [3],
            explanation: 'Archive Storage offers the lowest storage price per GB for data that is rarely accessed, typically less than once a year. While retrieval costs are higher, it is the most economical choice for long-term archival data.',
            resourceLink: 'https://cloud.google.com/storage/docs/storage-classes'
        },
        {
            id: 't3q31',
...
const TestSelection = ({ onSelectTest }) => {
    ...
    const handleStartTest = (testKey) => {
        onSelectTest(testKey, selectedMode);
    };
            ...
            <div className="max-w-6xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-12">
                    <h1 className="text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600 mb-4">
                        Google Cloud Associate Engineer Exam Simulator
                    </h1>
                    <p className="text-xl text-gray-600 max-w-2xl mx-auto">
                        Master your GCP knowledge with our comprehensive practice tests
                    </p>
                </div>

                {/* Mode Selection Card */}
                <div className="bg-white rounded-2xl shadow-xl p-8 mb-12 max-w-2xl mx-auto border-t-4 border-indigo-500 transform transition-all duration-300 hover:shadow-2xl">
                    <h2 className="text-2xl font-bold text-gray-800 mb-6 text-center">Select Your Mode</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        >
                            <div className="flex items-center justify-center space-x-3">
                                <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${
                                    selectedMode === 'exam' ? 'border-blue-500 bg-blue-500' : 'border-gray-400'
                                }`}>
                                    {selectedMode === 'exam' && (
                                ...
                                <span className={`text-lg font-semibold ${
                                    selectedMode === 'exam' ? 'text-blue-700' : 'text-gray-700'
                                }`}>Exam Mode</span>
                            </div>
                        ...
                        >
                            <div className="flex items-center justify-center space-x-3">
                                <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${
                                    selectedMode === 'practice' ? 'border-green-500 bg-green-500' : 'border-gray-400'
                                }`}>
                                    {selectedMode === 'practice' && (
                                        <div className="w-3 h-3 bg-white rounded-full"></div>
                                    )}
                                ...
                                <span className={`text-lg font-semibold ${
                                    selectedMode === 'practice' ? 'text-green-700' : 'text-gray-700'
                                }`}>Practice Mode</span>
                            </div>
                            <p className="mt-3 text-sm text-gray-600 text-center">
                                Learn with immediate feedback and detailed explanations
                            </p>
                        </button>
                    </div>
                </div>
                    ...
                    {['Test 1', 'Test 2', 'Test 3'].map((testName, index) => (
                        ...
                        >
                            <div className="text-white">
                                <h3 className="text-2xl font-bold mb-2">{testName}</h3>
                                <p className="text-blue-100 text-sm">
                                    {index === 0 ? 'Core Concepts' :
                                     index === 1 ? 'Advanced Topics' :
                                     'Expert Level'}
                                </p>
                                <div className={`absolute bottom-4 right-4 transition-transform duration-300 ${
                                    hoveredTest === index ? 'translate-x-0' : 'translate-x-2'
                                }`}>
                ...
                <p className="mt-12 text-center text-gray-500 text-sm">
                    Choose a test and mode to begin your GCP certification journey
                </p>
            </div>
        </div>
    );
...
const Test = ({ questions, onTestSubmit, onBackToSelection, testMode }) => {
    ...
    const [timeLeft, setTimeLeft] = useState(testMode === 'exam' ? 3600 : null);
    const [isLoading, setIsLoading] = useState(false);
    const [showAnswerFeedback, setShowAnswerFeedback] = useState(false);
    const [elaboratedExplanation, setElaboratedExplanation] = useState(null);
    ...
    const handleElaborateExplanation = async () => {
        ...
        const correctOptionsText = currentQuestion.correctAnswer.map(idx => currentQuestion.options[idx]).join(', ');
        const promptContent = `You are a Google Cloud expert providing in-depth explanations. Elaborate further on the core Google Cloud concept discussed in the following question and its provided explanation. Focus on clarifying any ambiguities or expanding on key terms. Do not repeat the question or the given explanation verbatim, but build upon it. Keep the explanation concise yet informative, suitable for someone studying for an Associate Cloud Engineer exam. 

        Question: ${currentQuestion.questionText}
        Correct Answer: ${correctOptionsText}
        Given Explanation: ${currentQuestion.explanation}
        
        Provide additional context, common use cases, or deeper technical insights related to the concept.`;
    ...
    const currentQuestion = questions[currentQuestionIndex];
    const isCurrentQuestionCorrect = isAnswerCorrect(currentQuestion, userAnswers[currentQuestionIndex]);

    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 flex items-center justify-center p-4 font-sans">
            <div className="bg-white rounded-2xl shadow-3xl p-8 w-full max-w-4xl border-t-[10px] border-blue-600">
                <div className="flex justify-between items-center mb-6 border-b pb-4">
                    <h2 className="text-3xl font-bold text-gray-800">
                        Question {currentQuestionIndex + 1} / {questions.length}
                    </h2>
                    {testMode === 'exam' && (
                        <div className="text-2xl font-semibold text-red-600 bg-red-100 px-4 py-2 rounded-lg shadow-md">
                    ...
                    {currentQuestion.options.map((option, index) => (
                        <label
                            ...
                            className={`flex items-center p-4 border rounded-lg cursor-pointer transition-colors duration-200 ease-in-out
                                ...
                                ${testMode === 'practice' && showAnswerFeedback && currentQuestion.correctAnswer.includes(index)
                                    ? 'bg-green-100 border-green-600 ring-4 ring-green-300'
                                    : ''
                                }
                                ${testMode === 'practice' && showAnswerFeedback && 
                                userAnswers[currentQuestionIndex] === index && 
                                !currentQuestion.correctAnswer.includes(index)
                                    ? 'bg-red-100 border-red-600 ring-4 ring-red-300'
                                    : ''
                                }
                            `}
                        >
                            <input
                                ...
                                disabled={testMode === 'practice' && showAnswerFeedback}
                                className="form-radio h-5 w-5 text-blue-600 focus:ring-blue-500 mr-4"
                            />
                ...
                {testMode === 'practice' && showAnswerFeedback && (
                    <div className={`p-5 mt-6 rounded-lg shadow-inner
                        ${isCurrentQuestionCorrect ? 'bg-green-50 border-2 border-green-400' : 'bg-red-50 border-2 border-red-400'}`}
                    >
                        ...
                        <p className="text-md text-gray-800 mb-2">
                            ...
                            {currentQuestion.correctAnswer.map(idx => currentQuestion.options[idx]).join(', ')}
                        </p>
                        <div className="text-md text-gray-700">
                            ...
                            {currentQuestion.resourceLink && (
                                <p className="mt-2 text-sm">
                                    <a href={currentQuestion.resourceLink} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline hover:text-blue-800 transition-colors duration-200">
                                        Learn more on Google Cloud Docs
                                    </a>
                                </p>
                            )}
                            <button
                                ...
                                disabled={isLoading}
                                className="mt-4 px-4 py-2 bg-purple-600 text-white font-semibold rounded-lg shadow-md hover:bg-purple-700 transition-all duration-200 flex items-center justify-center text-sm"
                            >
                                {isLoading ? (
                            ...
                            {elaboratedExplanation && (
                                <div className="mt-4 p-4 bg-gray-100 rounded-md border border-gray-200 text-gray-800">
                                    <h5 className="font-semibold mb-1">Elaborated Explanation:</h5>
                                    <p>{elaboratedExplanation}</p>
                                </div>
                            )}
                        </div>
                    </div>
                ...
                <div className="flex justify-between items-center pt-4 border-t mt-8">
                    <div className="flex gap-4">
                        <button
                            ...
                            className="px-6 py-3 rounded-lg font-semibold bg-gray-500 hover:bg-gray-600 text-white shadow-md hover:shadow-lg transform hover:scale-105 transition-all duration-200"
                        >
                            Home
                        </button>
                        <button
                    ...
                    {currentQuestionIndex < questions.length - 1 ? (
                        ...
                        <button
                            ...
                            className="px-6 py-3 rounded-lg font-semibold bg-green-600 hover:bg-green-700 text-white shadow-md hover:shadow-lg transform hover:scale-105 transition-all duration-200"
                        >
                            Submit Test
                        </button>
                    )}
...
// Helper function to check if an answer is correct
const isAnswerCorrect = (question, userAnswer) => {
    if (!userAnswer || userAnswer.length === 0) return false; // No answer provided
    if (userAnswer.length !== question.correctAnswer.length) return false; // Different number of selected options
    return userAnswer.every(val => question.correctAnswer.includes(val)); // All selected match all correct
};
...
const Results = ({ questions, userAnswers, score, onRetakeTest, onBackToSelection, testMode }) => {
    ...
    const handleElaborateExplanation = async (question) => {
        if (elaboratedExplanations[question.id]) return;
        
        setIsLoading(true);
        try {
            const explanation = await callGeminiAPI(
                `Please provide a detailed explanation for this GCP question: ${question.questionText}\n\n` +
            ...
            console.error('Error getting elaborated explanation:', error);
        }
        setIsLoading(false);
    };
    ...
    return (
        <div className="results-container">
                ...
                {questions.map((question, index) => (
                    <div key={question.id} className="question-container">
                        <div className="question-header" onClick={() => toggleQuestion(question.id)}>
                            ...
                            <span className={`status ${isAnswerCorrect(question, userAnswers[index]) ? 'correct' : 'incorrect'}`}>
                                {isAnswerCorrect(question, userAnswers[index]) ? '✓ Correct' : '✗ Incorrect'}
                            </span>
                        </div>
                                ...
                                <div className="explanation">
                                    ...
                                    {!elaboratedExplanations[question.id] && (
                                        <button
                                            ...
                                            disabled={isLoading}
                                        >
                                            {isLoading ? 'Loading...' : 'Get Detailed Explanation'}
                                        </button>
                                    )}
                                    {elaboratedExplanations[question.id] && (
                                        <div className="elaborated-explanation">
                                            <h4>Detailed Explanation:</h4>
                                            <p>{elaboratedExplanations[question.id]}</p>
                                        </div>
                                    )}
                                </div>
                            </div>
            ...
            <div className="nav-buttons">
                <button className="secondary-button" onClick={onBackToSelection}>
                    Back to Test Selection
                </button>
                <button className="primary-button" onClick={onRetakeTest}>
                    Retake Test
                </button>
            </div>
        </div>
...
const App = () => {
    const [currentView, setCurrentView] = useState('selection'); // 'selection', 'test', 'results'
    const [selectedTestKey, setSelectedTestKey] = useState(null); // Renamed for clarity
    ...
    const handleSelectTest = (testKey, mode) => {
        setTestQuestions(questionsData[testKey]);
        setSelectedTestKey(testKey);
    ...
    const handleTestSubmit = (answers) => {
        ...
        testQuestions.forEach(q => {
            ...
            if (isAnswerCorrect(q, userAnswer)) { // Use the helper function
                correctCount++;
            }
        });
        setScore(Math.round((correctCount / testQuestions.length) * 100));
        setCurrentView('results');
    };

    const handleRetakeTest = () => {
        // This effectively restarts the current test with the same mode
        handleSelectTest(selectedTestKey, testMode);
    };
    ...
    const handleBackToSelection = () => {
        ...
        setUserAnswers({});
        setScore(0);
        setTestMode('exam'); // Reset mode to default for selection screen
    };
    ...
    return (
        <div className="font-sans antialiased text-gray-900 bg-gray-100">
            ...
            {currentView === 'test' && selectedTestKey && (
                <Test
            ...
            {currentView === 'results' && (
                <Results
                    ...
                    onBackToSelection={handleBackToSelection}
                    testMode={testMode} // Pass testMode to results for conditional filtering
                />
            )}
        </div>
    );
